...
parser.add_argument("--checkpoint", type=str,
                    default="best", help="best or final")
...
def main(args):
    ...
    test_image_paths = glob.glob(
        "/kaggle/input/blood-vessel-segmentation/test/*/images/*.tif")
    ...
    for i in range(0, len(pred_image_paths), batch_size):
        start_index = i
        end_index = i + batch_size

        if end_index > len(pred_image_paths):
            end_index = len(pred_image_paths)

        preds = predictor.predict_from_files(pred_image_paths[start_index:end_index], None,
                                             save_probabilities=True, overwrite=True,
                                             num_processes_preprocessing=2,
                                             num_processes_segmentation_export=2,
                                             folder_with_segs_from_prev_stage=None,
        ...
        predicted_segmentations.extend(preds)

    print("Inference finished:", len(predicted_segmentations))
    with open(output_path, "wb") as f:
        pickle.dump(predicted_segmentations, f,
